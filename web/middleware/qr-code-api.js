...
import { getQrCodeOr404, getShopUrlFromSession, parseQrCodeBody, formatQrCodeResponse } from '../helpers/qr-codes.js';
  ...
  app.patch('/api/qrcodes/:id', async (req, res) => {
    ...
    if (qrcode) {
      try {
        ...
        const response = await formatQrCodeResponse(req, res, [await QRCodesDB.read(req.params.id)]);
