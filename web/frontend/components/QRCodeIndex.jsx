  ...
  return (
    <UnstyledLink onClick={() => navigate(`/qrcodes/${id}`)}>
      <div style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #E1E3E5' }}>
        <Stack>
          <Stack.Item>
            <Thumbnail source={product?.images?.edges[0]?.node?.url || ImageMajor} alt="placeholder" color="base" size="small" />
          </Stack.Item>
          <Stack.Item fill>
            <Stack vertical={true}>
              <Stack.Item>
...
export function QRCodeIndex({ QRCodes, loading }) {
  ...
  const smallScreenMarkup = QRCodes.map((QRCode) => <SmallScreenCard key={QRCode.id} navigate={navigate} {...QRCode} />);
      ...
      >
        ...
        <IndexTable.Cell>
          <Stack>
            ...
            <TextStyle variation={deletedProduct ? 'negative' : null}>{truncate(product?.title, 25)}</TextStyle>
          </Stack>
        </IndexTable.Cell>
        <IndexTable.Cell>{discountCode}</IndexTable.Cell>
        <IndexTable.Cell>{dayjs(createdAt).format('MMMM D, YYYY')}</IndexTable.Cell>
    ...
    <Card>
      {isSmallScreen ? (
        ...
        <IndexTable
          ...
          headings={[{ title: 'Thumbnail', hidden: true }, { title: 'Title' }, { title: 'Product' }, { title: 'Discount' }, { title: 'Date created' }, { title: 'Scans' }]}
          selectable={false}
          loading={loading}
        >
          {rowMarkup}
